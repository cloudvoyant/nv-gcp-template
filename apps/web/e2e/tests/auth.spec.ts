...
test.describe("Authentication", () => {
  ...
  test("logout clears session and redirects to Kinde", async ({ page }) => {
    await page.goto("/auth/logout");
    // Kinde logout endpoint redirects back to the app's origin after logout.
    // Wait for the full redirect chain to settle back on localhost.
    await page.waitForURL(/localhost/, { timeout: 10_000 });
